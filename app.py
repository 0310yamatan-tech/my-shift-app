...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v3.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v3.json")
...
# ーーー 💡【新機能】特定の曜日だけ人数を減らす（仕事が少ない日対策）設定 ーーー
st.markdown("##### 📉 特定の曜日だけ仕事の量を減らす（お休みを増やす）設定")
special_dow_settings = {}
with st.expander("ここをクリックして、仕事が少ない曜日・特別な人数の設定をする（任意）", expanded=False):
    st.caption("例：『水曜日は仕事が少ないからA作業を2人に、B作業を1人にする（結果的にお休みが3人⇒4人に増える）』といった設定が可能です。")
    for d_name in DAYS_BASE_WEEK:
        st.markdown(f"**【{d_name}曜日】の設定**")
        sc1, sc2, sc3 = st.columns(3)
        with sc1:
            is_special = st.checkbox(f"{d_name}曜日の出勤人数を通常と変える", key=f"sp_chk_{d_name}")
        if is_special:
            with sc2:
                sp_na = st.number_input(f"┗ {d_name}曜日の 🔴 A作業人数", min_value=1, max_value=int(n_staff), value=int(n_a)-1 if int(n_a)>1 else 1, key=f"sp_na_{d_name}")
            with sc3:
                sp_nb = st.number_input(f"┗ {d_name}曜日の 🔵 B作業人数", min_value=0, max_value=int(n_staff), value=int(n_b), key=f"sp_nb_{d_name}")
            special_dow_settings[d_name] = {"A": sp_na, "B": sp_nb}
...
for d_key in DAYS:
    if d_key in HOLIDAYS:
        ...
        if weekday_part in special_dow_settings:
            sa = special_dow_settings[weekday_part]["A"]
            sb = special_dow_settings[weekday_part]["B"]
            day_requirements[d_key] = {"A": sa, "B": sb, "Off": int(n_staff) - (sa + sb)}
        else:
        ...
        if day_requirements[d_key]["A"] + day_requirements[d_key]["B"] > n_staff:
            st.error(f"⚠️ エラー：{weekday_part}曜日の必要人数（合計 {day_requirements[d_key]['A'] + day_requirements[d_key]['B']}人）が、スタッフ全体の人数（{n_staff}人）を超えています。")
            st.stop()
...
if n_staff > 0:
    ...
    if rem == 0:
        ...
        st.info(
            f"💡 今回必要な出勤枠は合計で **{total_workdays}回分** です（仕事の少ない日も考慮済み）。"
...
# ーーー 💡【変更箇所】お名前を長谷川、羽田、遠藤、大竹、石井、澤田に設定 ーーー
...
def validate_fixed_rules(members, days_base, fixed_rules, day_reqs, max_consecutive, blocks):
    errors = []
    # 曜日ごとの固定人数が、その曜日の最大枠を超えていないかチェック
    for d in days_base:
        ...
        if sample_day:
            ...
            if len(fixed_a) > req["A"]:
                errors.append(f"「{d}曜日」に『🔴A固定』が {len(fixed_a)}人 指定されていますが、この曜日のA作業枠は {req['A']}人 しかないため計算できません。")
            if len(fixed_b) > req["B"]:
                errors.append(f"「{d}曜日」に『🔵B固定』が {len(fixed_b)}人 指定されていますが、この曜日のB作業枠は {req['B']}人 しかないため計算できません。")
            if len(fixed_off) > req["Off"]:
                errors.append(f"「{d}曜日」に『休 固定』が {len(fixed_off)}人 指定されていますが、この曜日にお休みできる上限（{req['Off']}人）を超えているため計算できません。")
...
if not validation_errors:
    if st.button("✨ 1ヶ月分のシフトを自動作成する", type="primary"):
        with st.spinner("コンピューターが一番良いシフトを計算しています。少しお待ちください..."):
            status, x, log = try_solve_with_relaxation(DAYS, HOLIDAYS, members, day_requirements, preferred_off, int(max_consecutive), balance, fixed_rules, blocks)
