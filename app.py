...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v5.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v5.json")
...
st.markdown("##### 👥 通常の日の配置人数（基本1人休みのバランスにしてください）")
col3, col4 = st.columns(2)
with col3:
    # 6人中1人休みにするため、A=4人、B=1人（計5人出勤）を初期値に設定
    n_a = st.number_input("🔴 通常の日の A作業に必要な人数", min_value=1, max_value=int(n_staff), value=4, step=1)
with col4:
    n_b = st.number_input("🔵 通常の日の B作業に必要な人数", min_value=0, max_value=int(n_staff), value=1, step=1)

# ーーー 💡【修正箇所】ピンポイントで特定の日付だけ「2人休み」にする設定（複数追加対応） ーーー
st.markdown("##### 📉 【日付限定】ピンポイントで仕事が少ない（2人休み）日の設定")
if "two_off_dates" not in st.session_state:
    st.session_state["two_off_dates"] = []

with st.expander("ここをクリックして、仕事が少なく【2人休み】にしたい日付を登録する（何個でも追加可能）", expanded=False):
    st.caption("『○月○日と、○月○日は仕事が少ないから2人休みにしたい』という日があれば、以下から1つずつ追加してください。通常の日より出勤人数が1人減ります。")
    ...
    if st.session_state["two_off_dates"]:
        st.write("**現在指定されている「2人休み」の日付一覧（何個でも登録できます）:**")
        for d_str in list(st.session_state["two_off_dates"]):
            ...
            with col_d_text:
                st.write(f"・{d_str}（通常より出勤枠を1人減らして2人休みにします）")
            with col_d_del:
...
for d_key in DAYS:
    if d_key in HOLIDAYS:
        ...
        # 💡2人休みにするため、通常の出勤人数（A+B）から1人減らす（A作業を1人減らします。Aが1人の場合はBを減らします）
        ...
        # 通常の日（入力された通り。A=4, B=1なら計5人出勤＝1人休み）
...
if n_staff > 0:
    ...
    if rem == 0:
        ...
        st.info(
            f"💡 今回必要な出勤枠は合計で **{total_workdays}回分** です（2人休みの日も計算に入っています）。"
