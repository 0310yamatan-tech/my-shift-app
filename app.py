...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v9.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v9.json")
...
with st.expander("ここをクリックして【希望休（特定の日付）】を設定・一括管理する", expanded=True):
    st.caption("混線を防むため、お休みを登録したい「スタッフ」と「日付」を選んで追加してください。")
