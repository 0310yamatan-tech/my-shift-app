...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v10.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v10.json")
...
# --- 🙅 希望休の設定（バグ防止のため、ボタン式を廃止しダイレクト選択へ変更） ---
...
with st.expander("ここをクリックして【希望休（特定の日付）】を登録する", expanded=True):
    ...
    for m in members:
        # スタッフごとにマルチセレクトを用意（キーに名前を含めることで混線を100%防止）
