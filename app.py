...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v6.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v6.json")
...
st.markdown("##### 👥 通常の日の配置人数（ご指定の人数に初期化しました）")
col3, col4 = st.columns(2)
with col3:
    # 💡ご要望通り通常の日のA作業人数を「3人」に変更
    n_a = st.number_input("🔴 通常の日の A作業に必要な人数", min_value=1, max_value=int(n_staff), value=3, step=1)
with col4:
    # 💡ご要望通り通常の日のB作業人数を「2人」に変更
    n_b = st.number_input("🔵 通常の日の B作業に必要な人数", min_value=0, max_value=int(n_staff), value=2, step=1)
...
with st.expander("ここをクリックして、仕事が少なく【2人休み】にしたい日付を登録する（何個でも追加可能）", expanded=False):
    st.caption("『○月○日と、○月○日は仕事が少ないから2人休みにしたい』という日があれば、以下から1つずつ追加してください。通常の日（出勤5人）より出勤人数が1人減って4人出勤になります。")
    ...
    if st.session_state["two_off_dates"]:
        ...
        for d_str in list(st.session_state["two_off_dates"]):
            ...
            with col_d_text:
                st.write(f"・{d_str}（通常よりA作業を1人減らして、計4人出勤＝2人休みにします）")
            with col_d_del:
...
for d_key in DAYS:
    if d_key in HOLIDAYS:
        ...
        # 💡2人休みにするため、通常(A=3, B=2)からA作業を1人減らして A=2, B=2 (計4人出勤＝2人休み) にします
