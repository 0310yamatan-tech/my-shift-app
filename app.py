...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v11.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v11.json")
...
with st.expander("ここをクリックして【希望休（特定の日付）】を登録する", expanded=True):
    st.caption("各スタッフがお休みしたい日付をすべて選択してください。ボタンを押さなくても自動で即時反映されます。")
    available_days = [d for d in DAYS if d not in HOLIDAYS]
    
    for m in members:
        selected_days_for_member = st.multiselect(
            f"👤 【{m}】さんの希望休を選択",
            options=available_days,
            key=f"ms_off_{m}"
        )
        for d_str in selected_days_for_member:
...
def try_solve_with_relaxation(days, holidays, members, day_reqs, preferred_off, max_consecutive, balance, fixed_rules, blocks):
    # 希望休を絶対に守った上で、出勤数の「公平さ（差）」の条件だけを段階的に緩める
    stages = [
