...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v4.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v4.json")
...
with col3:
    n_a = st.number_input("🔴 洗浄エリアに必要な人数", min_value=1, max_value=int(n_staff), value=3, step=1)
with col4:
    n_b = st.number_input("🔵 クリーンエリアに必要な人数", min_value=0, max_value=int(n_staff), value=1, step=1)

# ーーー 💡【新機能】ピンポイントで特定の日付だけ仕事が少ない日（4人休み）にする設定 ーーー
st.markdown("##### 📉 【日付限定】4人休みの日の設定")
if "less_work_dates" not in st.session_state:
    st.session_state["less_work_dates"] = []

with st.expander("ここをクリックして、仕事が少なく4人休みにしたい日付を指定する（任意）", expanded=False):
    st.caption("毎週ではなく『○月○日だけ仕事が少ない』という日があれば、以下から追加してください。自動的に残った2人がAに1人、Bに1人振り分けられます（4人休み）。")
    
    # 選択肢として今回のシフト期間内の日付リストを用意
    date_options = {DAY_TO_DATE[d]: d for d in DAYS if d not in HOLIDAYS}
    sorted_dates = sorted(list(date_options.keys()))
    
    if sorted_dates:
        selected_target_date = st.selectbox("仕事が少ない日を選択", options=sorted_dates, format_func=lambda x: x.strftime('%m/%d'))
        if st.button("➕ この日付を4人休みの日に指定する"):
            day_key_str = date_options[selected_target_date]
            if day_key_str not in st.session_state["less_work_dates"]:
                st.session_state["less_work_dates"].append(day_key_str)
                st.toast(f"📢 {selected_target_date.strftime('%m/%d')} を4人休みの日に指定しました！")
    
    if st.session_state["less_work_dates"]:
        st.write("**現在の指定済み日程（残った2人を洗浄エリアとクリーンエリアに1人ずつ配置）:**")
        for d_str in list(st.session_state["less_work_dates"]):
            # シフト期間外になった古いデータは除外
            if d_str not in DAYS:
                st.session_state["less_work_dates"].remove(d_str)
                continue
...
for d_key in DAYS:
    ...
    elif d_key in st.session_state["less_work_dates"]:
        # 💡ご要望通り、4人休みにして残った2人をAとBに1人ずつ振り分け
        day_requirements[d_key] = {"A": 1, "B": 1, "Off": 4}
    else:
...
if n_staff > 0:
    ...
    if rem == 0:
        st.info(f"💡 今回の期間（合計出勤枠 {total_workdays}回分）では、全員がちょうど **{base}日ずつ** 出勤すればピッタリ均等になります。")
    else:
        st.info(
            f"💡 今回必要な出勤枠は合計で **{total_workdays}回分** です（仕事の少ない日も計算に入っています）。"
...
with st.expander("ここをクリックして各スタッフの曜日固定を設定する", expanded=False):
    for m in members:
        ...
        for d, c in zip(DAYS_BASE_WEEK, cols):
            with c:
                choice = st.selectbox(f"{d}曜日", options=["指定なし", "🔴 洗浄エリア固定", "🔵 クリーンエリア固定", "休 固定"], key=f"fix_{m}_{d}")
                if choice == "🔴 洗浄エリア固定": fixed_rules[m][d] = "A"
                elif choice == "🔵 クリーンエリア固定": fixed_rules[m][d] = "B"
                elif choice == "休 固定": fixed_rules[m][d] = "Off"

# --- 🙅 希望休の設定 ---
st.subheader("🙅 4. 希望休の設定")
preferred_off = {m: set() for m in members}
with st.expander("ここをクリックして希望休を設定する", expanded=False):
    for m in members:
    ...
    for d in days_base:
        ...
        if sample_day:
            ...
            if len(fixed_a) > req["A"]:
                errors.append(f"「{d}曜日」に『🔴洗浄エリア固定』が {len(fixed_a)}人 指定されていますが、最大枠は {req['A']}人 です。")
            if len(fixed_b) > req["B"]:
                errors.append(f"「{d}曜日」に『🔵クリーンエリア固定』が {len(fixed_b)}人 指定されていますが、最大枠は {req['B']}人 です。")
