...
st.markdown("##### 📉 【日付限定】ピンポイントで仕事が少ない（4人休み）日の設定")
...
with st.expander("ここをクリックして、仕事が少なく4人休みにしたい日付を指定する（任意）", expanded=False):
    ...
    if st.session_state["less_work_dates"]:
        st.write("**現在の指定済み日程（残った2人をAとBに1人ずつ配置）:**")
    ...
    for d in days_base:
        ...
        if sample_day:
            ...
            if len(fixed_a) > req["A"]:
                errors.append(f"「{d}曜日」に『🔴A固定』が {len(fixed_a)}人 指定されていますが、最大枠は {req['A']}人 です。")
            if len(fixed_b) > req["B"]:
                errors.append(f"「{d}曜日」に『🔵B固定』が {len(fixed_b)}人 指定されていますが、最大枠は {req['B']}人 です。")
