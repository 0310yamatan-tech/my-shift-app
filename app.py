...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v12.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v12.json")
...
# --- 🙅 希望休の設定（★データがリセットされないよう完全セッション管理化） ---
st.subheader("🙅 4. 希望休の設定")

if "perfect_preferred_offs" not in st.session_state:
    st.session_state["perfect_preferred_offs"] = {}

# 現在のメンバー構成に合わせてセッション辞書をクリーン＆初期化
current_offs_dict = {}
for m in members:
    if m not in st.session_state["perfect_preferred_offs"]:
        st.session_state["perfect_preferred_offs"][m] = []
    current_offs_dict[m] = st.session_state["perfect_preferred_offs"][m]
...
with st.expander("ここをクリックして【希望休（特定の日付）】を登録する", expanded=True):
    ...
    for m in members:
        # セッションに保存されている過去の選択値を初期値としてセット
        default_selected = [d for d in current_offs_dict[m] if d in available_days]
...
if not validation_errors:
    if st.button("✨ 1ヶ月分のシフトを自動作成する", type="primary"):
        with st.spinner("コンピューターが一番良いシフトを計算しています..."):
            ...
            # 安全に同期されたfinal_preferred_offを渡す
