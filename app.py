...
def build_and_solve(**params):
    ...
    # 目的関数：公平性の差を最小化しつつ、割り当てをランダムに散らすための微小なペナルティを追加
    ...
    # 【デバッグ箇所】土日の有無に合わせて連勤判定ブロックを動的に切り替え
    if enforce_consec:
        if include_weekend:
            check_blocks = [DAYS]
        else:
...
def try_solve():
    stages = [
        ...
        {"label": "⑤ 条件緩和（人数優先・ランダム作成）", "enforce_fair_work": False, "fair_tol": 999.0, "enforce_consecutive": False},
    ]
    log = []
    base_params = {
