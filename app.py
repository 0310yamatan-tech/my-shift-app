...
# ------------------------------------------------------------------
import tempfile
BALANCE_FILE = os.path.join(tempfile.gettempdir(), "shift_balance_v8.json")
TABLE_FILE = os.path.join(tempfile.gettempdir(), "shift_table_v8.json")
...
with st.expander("ここをクリックして【希望休（曜日・特定の日付）】を設定・一括管理する", expanded=True):
    
    st.markdown("#### 📅 1. 曜日での希望休設定")
    for m in members:
        st.write(f"**【{m}】さんの固定休み曜日:**")
        cols = st.columns(len(DAYS_BASE_WEEK))
        for d, c in zip(DAYS_BASE_WEEK, cols):
            with c:
    ...
    st.markdown("---")
    st.markdown("#### 📌 2. ピンポイントでの希望休登録（日付指定）")
    st.caption("混線を防ぐため、お休みを登録したい「スタッフ」と「日付」を1組ずつ選んで追加してください。")
    ...
    if sorted_valid_dates:
        col_p_staff, col_p_date, col_p_btn = st.columns([2, 2, 1])
        with col_p_staff:
            selected_m = st.selectbox("スタッフを選択", options=members, key="p_staff_select")
        with col_p_date:
            selected_d = st.selectbox("お休みさせたい日を選択", options=sorted_valid_dates, format_func=lambda x: x.strftime('%m/%d'), key="p_date_select")
        with col_p_btn:
            st.write("")
            if st.button("➕ この希望休をリストに追加", type="secondary", use_container_width=True):
                target_day_str = valid_date_options[selected_d]
                if target_day_str not in st.session_state["individual_off_dates"][selected_m]:
                    st.session_state["individual_off_dates"][selected_m].append(target_day_str)
                    st.toast(f"📢 {selected_m}さんの希望休に {selected_d.strftime('%m/%d')} を追加しました！")
                    st.rerun()

    # 登録済みのピンポイント希望休一覧の表示と削除
    st.write("**【現在登録されている日付指定の希望休一覧】**")
    has_any_indiv = False
    for m in members:
        m_saved_dates = st.session_state["individual_off_dates"].get(m, [])
        if m_saved_dates:
            has_any_indiv = True
            st.markdown(f"👤 **{m} さんの希望休:**")
            for d_str in list(m_saved_dates):
                if d_str not in DAYS:
                    st.session_state["individual_off_dates"][m].remove(d_str)
                    continue
                
                # 計算用変数preferred_offにも確実に同期させる
                ...
                with c_del:
                    if st.button("🗑️ 取消", key=f"del_indiv_{m}_{d_str}"):
                        st.session_state["individual_off_dates"][m].remove(d_str)
                        st.rerun()
    if not has_any_indiv:
        st.caption("現在、特定の日付に対する希望休は登録されていません。")
