...
# --- 🙅 希望休の設定（曜日指定 ＋ 💡【新機能】ピンポイント日付指定） ---
st.subheader("🙅 4. 希望休の設定（お休みにしたい曜日や日付を指定・任意）")
preferred_off = {m: set() for m in members}

# セッション状態に各個人の個別希望休リストを作っておく
    ...
    for m in members:
        ...
        if m_saved_dates:
            ...
            for d_str in list(m_saved_dates):
                ...
                preferred_off[m].add(d_str) # 計算用セットに合流させる
